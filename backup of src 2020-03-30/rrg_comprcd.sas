...
%macro rrg_comprcd(rrguri=, oldlib=, newlib=)/store;

%local rrguri oldlib newlib;

%* oldlib: path to folder with previously generated rcd;
%* newlib: path to folder with newwly generated rcd;
%* rrguri: name of rcd file;

libname __oldlb "&oldlib";
libname __newlb "&oldlib";
...
%if &vnum1>0 and &vnum2>0 %then %do;
  ...
  proc sql noprint;
  select max(__dsid) into:oldnumds separated by '' from __oldlb.&rrguri;
  select max(__dsid) into:newnumds separated by '' from __newlb.&rrguri;
  quit;
