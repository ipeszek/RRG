...
*  LABEL_CI        = display label for odds ratio and CI
...
%local dataset where trtvar groupvars  var  refvals subjid oddsfmt 
       alpha label_or label_ci label_orci;
%LOCAL maxtrt;
   ...
   %do i=1 %to &ntrts;
    ...
    %* keep only cases where there is data in each of 4 cells;
        ...
        proc sort data=__fishtmp ;
          by &groupvars __tmpgb __tmptrt __out;
        run;
        ...
        data __cnttmp;
          ...
          keep &groupvars __tmpgb __cnttmp;
        run;
        ...
        %end;
         ...
         %end;
           weight __wt;
           tables __tmptrt*__out/relrisk alpha=&alpha;
         run;
...
%make_est:      
         ...
         length __stat_name __stat_label __or __ci __stat_value __stat_align $ 200;
          ...
          if uppercl ne . then __stat_value = cats("(", __stat_value)||", "
             ||cats(put(uppercl, &oddsfmt), ")");
