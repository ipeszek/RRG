...
proc sql noprint;
  ...
  /*select dist2next into:dist2next separated by ' ' from __repinfo;*/
...
%if &savercd=Y /*and %upcase(&print)=Y*/  %then %do;
  %__savercd;
%end;

%if %upcase(&gentxt)=Y /*and %upcase(&print)=Y */ %then %do;
...
%if %upcase(&java2sas)=Y  %then %do;
  ...
  * call symput ('numcol', strip(put(numcol, best.)));
  ...
  /*
  %local dsid rc varnum ispage;
  %let ispage=0;
  %let dsid = %sysfunc(open(&rrguri));
  %let varnum = %sysfunc(varnum(&dsid, __varbygrp));
  %if &varnum>0 %then %let ispage=1;
  %let rc = %sysfunc(close(&dsid)); 
  */
  %local ispage;
  %let ispage=0;
...
/*
%if &tablepart=LAST or %length(&tablepart)=0  %then %do;
proc optload 
