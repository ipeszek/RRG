...
%local  datasetRRG   tablabel
       ...
       libsearch  varby4pop varbyn4pop groupby4pop groupbyn4pop groupby4ae groupbyn4ae splitcolumns;
...
proc sql noprint;
    ...
    select count(*), name    into :nnotcgrpv  , :notcgroupby separated by ' '
    from __varinfo(where=(upcase(type)='GROUP' and upcase(page) ne 'Y' and upcase(across) ne 'Y' ));
  ...
  select name into:groupbyn4ae separated by ' '
    from __varinfo(where=(upcase(type)='GROUP' and upcase(page) ne 'Y' and upcase(aegroup) ne 'Y'));  

 select name into:groupby4ae separated by ' '
    from __varinfo(where=(upcase(type)='GROUP' and upcase(page) ne 'Y' and upcase(aegroup) = 'Y'));      
...
%put DEBUG INFO: ngrpv=&ngrpv nnotcgrpv=&nnotcgrpv;

%local  cgrps cgrps_w_trt tmp notcgrps allgrps ;
...
%put DEBUG INFO: groupvars4ae passed to __cnts macro: &cgrps &notcgrps;
%put DEBUG INFO: groupvarsn4ae passed to __cnts macro: &groupbyn4ae;
%put DEBUG INFO: groupby4pop=&groupby4pop;
%put DEBUG INFO: groupbyn4pop=&groupbyn4pop;
      ...
      groupvarsn4pop = &groupbyn4pop,
        groupvars4ae = &groupby4ae,  
/*        groupvars4ae = &cgrps &notcgrps, */
      groupvarsn4ae = &groupbyn4ae,      
             byn4pop = &varbyn4pop ,
              by4pop = &varby4pop ,
...
record=  "if 0 then __sid=1;"; output;
record=  "run;"; output;
record=  " "; output;
record=  "*------------------------------------------------------------------;"; output;
...
%if &ngrpv>0 or &nvarby>0 %then %do;
    ...
    %end;
      ...
      %do i=1 %to &nnotcgrpv;  
           %local ncgrp&i  ncgrplab&i tmp;  
           %let ncgrp&i = %scan(&notcgroupby, &i, %str( ));  
           %let tmp = &&ncgrp&i;  
           %local grpdec_&&ncgrp&i;  
           select distinct decode, label into :grpdec_&&ncgrp&i separated by ' ',  
           :ncgrplab&i separated by ' '  
            from __varinfo where upcase(name)=upcase("&&ncgrp&i")  
                 and type='GROUP' and page ne 'Y' and upcase(across) ne 'Y';  
    
      %end;  

    %do i=1 %to &nvarby;
         %local vby&i  vblabel&i vnline&i;
         %let vby&i = %scan(&varby, &i, %str( ));
         %let tmp = &&vby&i;
         %local vbdec_&&vby&i;
         select distinct decode, label, nline  into
           :vbdec_&&vby&i separated by ' ',
           :vblabel&i  separated by ' ',
           :vnline&i  separated by ' '
    ...
    %if %length(&varby4pop) %then %do;
      %let varby4pop=%sysfunc(compbl(&varby4pop));
      data rrgpgmtmp;
      length record $ 2000;
      keep record;
      record=  " ";  output;
      record=  "*---------------------------------------------------------------;"; output;
      record=  "* Get count for page-by variables;";   output;
      record=  "*---------------------------------------------------------------;"; output;
      record=  " ";  output;
      record=  "proc sql;                                                                                               "; output;
      record=  "create TABLE __n4varby as select %sysfunc(tranwrd(&varby4pop, %str( ), %str(,))), count(*) as __nlinevb "; output;
      record=  "from (select distinct %sysfunc(tranwrd(&defreport_subjid &varby4pop, %str( ), %str(,))) from __dataset )                  "; output;
      record=  "group by %sysfunc(tranwrd(&varby4pop, %str( ), %str(,)));                                               "; output;
      record=  "quit  ;                                                                                                  "; output;
      record=  " ";  output; 
      record=  "proc sql nowarn UNDO_POLICY=NONE;"; output;
      record=  "  create table __all as select * from __all natural left join __n4varby;"; output;
      record=  "quit;"; output;
      record=  " ";  output;
    ...
    %if %sysfunc(exist(__grpcodes_exec)) %then %do;
     ...
     %if &ngrpv=0 %then %do;
      ...
      %put DEBUG INFO: case 1, no grouping variables;
         ...
         record=  " ";                                                                                   output;
         record=  "*---------------------------------------------------------;";                         output;
         record=  "* CREATE __VARBYLAB WITH DESCRIPTION OF PAGE-BY VARIABLES  ;";                         output;
         record=  "*---------------------------------------------------------;";                         output;
         record=  " ";                                                                                   output;
         ...
         %do j=1 %to &nvarby;
              ...
              %end;
              %if %upcase(&&vnline&j)=Y %then %do;
                  record=  "   __varbylab=trim(left(__varbylab))||' (N='||strip(put(__nlinevb, best.))||')';";                       output;
              %end;
    ...
    %if &ngrpv>0 and %length(&grps_no_cl)=0 %then %do;
      ...
      %put DEBUG INFO: case 3, all grouping variables have  codelies;
    ...
    %if &ngrpv>0 and  %length(&grps_w_cl)>0 and %length(&grps_no_cl)>0 %then %do;
        ...
        %put DEBUG INFO: case 4, some but not all  grouping variables have  codelies;
...
%*-------------------------------------------------------------------;
 ...
 record = '                                                                                                                                        '; output;                                    
 record = '%local numstat r1 r2 j ; '; output;
 record = '%let numstat=1;'; output;
 run;
...
%if %upcase(&defreport_statsacross)=Y and (%upcase(&defreport_aetable)=N  or &defreport_ae_max=Y )  %then %do;
...
/*  */
/* %else %if %upcase(&defreport_statsacross)=Y and %upcase(&defreport_aetable) ne N and &splitcolumns ne 'Y' %then %do; */
/*  */
/* %* this was added to allow >1 stat in AE tables; */
/* %* this will be superseeded by splitting columns when splitcolumns=Y;      */
/*                 */
/*       data rrgpgmtmp; */
/*       length record $ 2000; */
/*       keep record; */
...
/*  */
/*       record = '                                                                                                                                        '; output; */
/*       record = '     data __numstat;'; output; */
/*       record = '      set sashelp.vcolumn;'; output; */
/*       record = '      where memname="__ALL" and libname="WORK" and lowcase(name)=:"__col" and lowcase(name) ne: "__cole" and lowcase(name) ne: "__col_";'; output; */
/*       record = '      tt=input(tranwrd(scan(name,1,"_"),"col",""),??best.);                                                                             '; output; */
/*       record = '      keep name tt;                                                                                                                     '; output; */
/*       record = '      run;                                                                                                                              '; output; */
/*       record = '                                                                                                                                        '; output; */
/*       record = '     proc sql noprint;                                                                                                                  '; output; */
/*       record = '     select max(tt) into: numstat separated by  " " from __numstat;                                                                     '; output; */
/*       record = '     quit;                                                                                                                              '; output; */
/*       record = '                                                                                                                                        '; output; */
/*       record = '                                                                                                                                        '; output; */
/*       record = '     %let r1 =;                                                                                                                         '; output; */
/*       record = '     %let r2 =;                                                                                                                         '; output; */
/*       record = '                                                                                                                                        '; output; */
/*       record = '                                                                                                                                        '; output; */
/*       record = '     %do i=1 %to &maxtrt;                                                                                                               '; output; */
/*       record = '       %let r1= &r1 __col_&i=__ncol_%eval(&i*&numstat-&numstat+1);                                                                      '; output; */
/*       record = '       %do j=2 %to &numstat;                                                                                                            '; output; */
/*       record = '         %let r1= &r1 __col&j._&i=__ncol_%eval(&i*&numstat-&numstat+&j);                                                                '; output; */
/*       record = '                                                                                                                                        '; output; */
/*       record = '      %end;                                                                                                                             '; output; */
/*       record = '     %end;                                                                                                                              '; output; */
/*       record = '     %do i=1 %to %eval(&maxtrt*&numstat);                                                                                               '; output; */
/*       record = '       %let r2=&r2 __ncol_&i=__col_&i;                                                                                                  '; output; */
/*       record = '     %end;                                                                                                                              '; output; */
/*       record = '                                                                                                                                        '; output; */
/*         */
/*       record = '      data __all (rename=(&r2));                                                                                                        '; output; */
/*       record = '       set __all (rename=(&r1));                                                                                                        '; output; */
/*       record = '      run;                                                                                                                              '; output; */
/*       record = '                                                                                                                                        '; output; */
/*       record = '                                                                                                                                        '; output; */
/*         */
...
record=  "  set __all;";                                                                                      output;
record=  "  if __sid=. then __sid=1;";                                                                        output;
...
%do i=1 %to &ngrpv;
    record=  "  if last.&&grp&i then __last_&&grp&i=1;"; output;
%end;
...
%if %upcase(&defreport_aetable) ne N %then %do;
    ...
    %do i=&nnotcgrpv %to 1 %by -1;

        record=  "         if first.&&ncgrp&i  then do;";                                                       output;

        %if %upcase(&defreport_statsacross)=Y and &ngrpv>0 %then %do;
           record=  "          if __grpid ne 999 then __nindentlev = __indentlev-&nnotcgrpv+__grpid-1;";          output;
           record=  "          else __nindentlev = __indentlev;";                                             output;
        %end;
        %else %do;
/*             record=  "          __nindentlev = __grpid-2+__indentlev-&nnotcgrpv;";                                output; */
        record=  "          __nindentlev = __grpid-2+__indentlev-&nnotcgrpv;";    output;
        %end;

        record=  "          __sid = -&nnotcgrpv+&i-1;";                                                           output;
        record=  "          __vtype=CATS('GLABEL',&i);";                                                      output;
        record=  "          if not last.&&ncgrp&i then do;";                                                    output;
        record=  "          __suffix='';";                                                                    output;
        record=  "          __keepn=1;";                                                                      output;
        record=  "          end;";                                                                            output;
        record=  "          __col_0=trim(left(__grplabel_&&ncgrp&i));";                                         output;
        record=  "         end;";                                                                             output;
    %end;

    %if &nnotcgrpv>0 %then %do;
...
%if %upcase(&defreport_statsacross)=Y and  %upcase(&defreport_aetable) ne N and %length(&splitcolumns) =0 %then %do;


    proc sql noprint;
      select statnames into: statnames from __statnames;
    quit;

    %if %sysfunc(countw(&statnames, %str( )))=1 %then %goto skipextrahead;
    
    %do i=1 %to %sysfunc(countw(&statnames, %str( )));
      %local stat&i;
      %let stat&i=%qscan(&statnames,&i,%str( ));   
    %end;
    ...
    record=  " ";                                                                                           output;
    record=  "*--------------------------------------------------------------------;";                      output;
    record=  "* CREATE STAT LABELS  HEADINGS ;";                                                             output;
    record=  "*--------------------------------------------------------------------;";                      output;
    record=  " ";                                                                                           output;
    ...
    record=  '  length __ncol_0 -__ncol_%eval(&numstat*&maxtrt)     $ 2000 __aestats $ 200;';               output;
    record=  "  retain __ncol_0;";                                                                          output;
    record=  '  __aestats=symget("aestats");';                                                              output;

    record=  '  %do i=1 %to &maxtrt;  ';                                                                    output;
    record=  '    %do j=1  %to &numstat; ';                                                                 output;
    
    record=  '    __ncol_%eval((&i-1)*&numstat+&j)=__col_&i;';                                               output;
    record=  '    %end;';                                                                                   output;
    record=  '  %end;';                                                                                     output;
    record=  " ";                                                                                           output;
    %if &nvarby=0 %then %do;
    ...
    record=  "           __rowid+1;";                                                                       output;
    record=  '           %do i=1 %to &numstat;  ';                                                          output;
    record=  '             %do j=1  %to &maxtrt; ';                                                         output;
    record=  '              __ncol_%eval((&j-1)*&numstat+&i)=put(scan(__aestats, &i), $__rrgsf.);';         output;
    record=  '             %end;';                                                                          output;
    record=  '           %end;';                                                                            output;
    record=  " ";                                                                                           output;
    record=  "           output;";                                                                          output;
    ...
    record=  "  run;";                                                                                      output;
    record=  " ";                                                                                           output;
    record=  '  data __head (drop=__align __i __j rename=( __nalign=__align ';                              output;
    record=  '  %do i=1 %to %eval(&numstat*&maxtrt); __ncol_&i=__col_&i %end;));';                          output;
    record=  '  set __head (drop=__col_1-__col_&maxtrt );';                                                 output;

      record=  "  length __nalign $ 2000;";                                                                   output;  
      record=  "    __nalign = scan(__align,1,' ');";                                                         output;  
      record=  '  do __i=1 to &maxtrt;';                                                                      output;  
      record=  '  do __j=1 to &numstat;';                                                                      output;  
      record=  "        __nalign = left(trim(__nalign))||' C';";                                            output;  
      record=  '  end;';                                                                                      output;  
      record=  '  end;';                                                                                      output;  
      record=  "  run;";                                                                                      output;
    record=  " ";                                                                                           output;

    record=  '  %let maxtrt = %eval(&numstat*&maxtrt);';                                                    output;
...
%* end of aetable and stats across;
%skipextrahead:
%end;
...
%* handle splitcolumns in __fall and __head;


%if %length(%nrbquote(&splitcolumns))  %then %do;
    record=  " ";                                                                                                 output;
    record=  "*--------------------------------------------------------;";                                        output;
    record=  "* SPLIT COLUMNS THAT COMTAIN MULTIPLE STATISTICS;";                                                 output;
    record=  "*--------------------------------------------------------;";                                        output;
    record=  " ";                                                                                                 output;  

    record=  " ";                               
    record=  'data __head(rename=(%do i=1 %to%eval(&maxtrt*2); __ncol_&i=__col_&i %end;)) ;    '; output;
    record=  'set __head end=eof;                                                              '; output;
    record=  '  length __ncol_1-__ncol_%eval(&maxtrt*2) __oldcol0  __statnames $ 2000;         '; output;
    record=  ' __statnames = compbl(upcase("' ||"&splitcolumns"||'"));';  output;
    record=  '  array ncols __ncol_1-__ncol_%eval(&maxtrt*2);                                  '; output;
    record=  '  array cols __col_1-__col_&maxtrt;                                              '; output;
    record=  '  do i=1 to &maxtrt;                                                             '; output;
    record=  '    ncols[(i-1)*2+1]=strip(cols[i]);                                             '; output;
    record=  '    ncols[i*2]=strip(cols[i]);                                                   '; output;
    record=  '  end;                                                                           '; output;
    record=  "  __align ='L'||repeat(' C', dim(ncols));";                                         output;
    record=  '  if eof then do;                                                                '; output;
    record=  '    __oldcol0=__col_0;                                                           '; output;
    record=  "    __col_0='';                                                                  "; output;
    record=  '    output;                                                                      '; output;
    record=  '    __rowid=__rowid+1;                                                           '; output;
    record=  '    __col_0=__oldcol0;                                                           '; output;
    record=  '    do i=1 to &maxtrt;                                                           '; output;
    record=  "      ncols[(i-1)*2+1]=put(scan(__statnames,1),__rrgsf.);                        "; output;
    record=  "      ncols[i*2]=put(scan(__statnames,2),__rrgsf.);                              "; output;
    record=  '    end;                                                                         '; output;
    record=  '    output;                                                                      '; output;
    record=  '  end;                                                                           '; output;
    record=  '  else output;                                                                   '; output;

    record=  '  drop __col_1-__col_&maxtrt __oldcol0 __statnames;                              '; output;
    record=  'run;                                                                             '; output;

    record=  ' ';                                 
                                                
    record=  'data __fall(rename=(%do i=1 %to%eval(&maxtrt*2); __ncol_&i=__col_&i %end;)) ;     '; output;
    record=  '  set __fall;                                                                     '; output;
    record=  '  length __ncol_1-__ncol_%eval(&maxtrt*2) $ 2000;                                 '; output;
    record=  '  array ncols __ncol_1-__ncol_%eval(&maxtrt*2);                                   '; output;
    record=  '  array cols __col_1-__col_&maxtrt;                                               '; output;
    record=  '  do i=1 to &maxtrt;                                                              '; output;
    record=  '                                                                                  '; output;
    record=  "    ncols[(i-1)*2+1]=strip(scan(cols[i],1,'!'));                                  "; output;
    record=  "    ncols[i*2]=strip(scan(cols[i],2,'!'));                                        "; output;
    record=  '  end;                                                                            '; output;
    record=  '  drop __col_1-__col_&maxtrt;                                                     '; output;
    record=  'run;                                                                              '; output;

    record=  ' ';    output;
    record=  '%let maxtrt=%eval(&maxtrt*2); ';      output;                           
    record=  ' ';    output;
...
record=  "* CREATE __SUFFIX IF __COL_0 ENDS WITH //;";                                                                     output;
record=  "* -----------------------------------------------------------------------;";                                                            output;
record=  " ";                                                                                                                                     output;
...
record=  "  if first.__datatype then __next_indentlev=.;";    
%if %length(&defreport_tablepart) %then %do;                                  output;
record=  "__tablepart=&defreport_tablepart;";                                                                                                     output;
%end;
%else %do;
  record=  "__tablepart=.;";                                                                                                     output;
%end;  
record=  "  if __datatype ne: 'R' and length(__col_0)>1 then do; "; output;
record=  "    if substr(strip(__col_0), length(strip(__col_0))-1)='//' then do; "; output;
record=  "      __suffix='~-2n';"; output;
record=  "      __col_0 = substr(strip(__col_0), 1, length(strip(__col_0))-2);"; output;
record=  "    end;";                                                             output;
record=  "  end;";                                                               output;
record=  "run;";                                                                 output;
record=  " ";                                                                                                                                     output;
